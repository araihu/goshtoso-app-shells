...
func TestRuntimeLifecycleContract(t *testing.T) {
	...
	for _, want := range []string{"htmx:before:swap", "htmx:after:settle", "sidebarScrollTop", "main.scrollTo({top:0})", "focusMain(main)", "reconcileNavigation(main)", "closeDrawer", "restoreFocus === false", "window.__consoleShellLifecycleInstalled", "target.matches(\"main.console-shell__main\")", "window.__consoleShellAlpineRegistered", "root.dataset.themeSource", "htmx.process", "document.title", "popstate"} {
