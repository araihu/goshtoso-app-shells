(function () {
  ...
  function registerAlpineData() {
    ...
    window.Alpine.data("consoleShell", function (o) {
      ...
      return { sidebarOpen: false, init: function () { document.documentElement.dataset.theme=theme; document.documentElement.classList.toggle("dark",dark); }, closeDrawer: function () { this.sidebarOpen=false; this.$nextTick(function () { if (this.$refs.sidebarTrigger) this.$refs.sidebarTrigger.focus(); }.bind(this)); } };
    });
  }
  ...
  function afterSwap(event) {
    ...
    titleFromSwap(event.detail.xhr && event.detail.xhr.responseXML);
  ...
  function installLifecycle() {
    ...
    window.addEventListener("popstate", function () { var main=document.querySelector("main.console-shell__main"); if (main) focusMain(main); });
  }
